...
CTD_IMPL_CONSOLE_CMD( CmdStartServer );


CmdStartServer::CmdStartServer() :
 BaseConsoleCommand( CMD_NAME_STARTSERVER )
{
    setUsage( CMD_USAGE_STARTSERVER );
}
...
const std::string& CmdStartServer::execute( const std::vector< std::string >& arguments )
{
    _cmdResult = "";
    if ( arguments.size() < 1 )
    {
