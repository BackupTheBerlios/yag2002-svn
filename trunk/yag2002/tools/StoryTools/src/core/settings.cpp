...
BEDITOR_SINGLETON_IMPL( SettingsManager )
...
bool Settings::load( const std::string& filename )
{
    ...
    for( size_t cnt = 0; cnt < tokens; ++cnt ) 
    {
        ...
        if ( settings_typeinfo == typeid( float ) ) {
         ...
         if ( settings_typeinfo == typeid( Eigen::Vector3f ) ) {
             read( p_setting->getTokenName(), static_cast< Setting< Eigen::Vector3f >* >( p_setting )->_value );
        } else 
...
bool Settings::store( const std::string& filename )
{
    ...
    for( size_t cnt = 0; cnt < tokens; ++cnt ) {
        ...
        if ( settings_typeinfo == typeid( Eigen::Vector3f ) ) {
            write( p_setting->getTokenName(), static_cast< Setting< Eigen::Vector3f >* >( p_setting )->_value );
        } else
            assert( NULL && "unsupported setting type" );
    }
...
bool Settings::read( const std::string& token, Eigen::Vector3f& value )
{
    std::string  strvalue;
    if ( read( token, strvalue ) == false ) 
        return false;
    ...
    std::stringstream strstream( strvalue );
    strstream >> value.x() >> value.y() >> value.z();
    return true;
}
...
bool Settings::write( const std::string& token, const Eigen::Vector3f& value )
{
    std::stringstream str;
    str << token << "=" << value.x() << " " << value.y() << " " << value.z() << "\r\n";
    _fileBuffer += str.str();
    return true;
}
