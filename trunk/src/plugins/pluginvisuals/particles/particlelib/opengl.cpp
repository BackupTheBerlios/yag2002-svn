...
// 
// 06/23/2003	extended by boto (botorabi@gmx.net) to adapt the particle system to fly3D engine
...
PARTICLEDLL_API void pDrawGroupl(int dlist, bool const_size, bool const_color, bool const_rotation)
{
	...
	for(int i = 0; i < pg->p_count; i++)
	{
		...
		if(!const_rotation)
		{
			...
			glMultMatrixf((float *)&g_flyengine->cam->mat); 	

			// commented out by boto to adapt the particle orientation to camera
			/*
			pVector vN(m.vel);
			vN.normalize();
			pVector voN(m.velB);
