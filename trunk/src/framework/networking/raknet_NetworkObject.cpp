...
//---------------------------------------------//
// CTD modification: begin the static id at 1  //
//---------------------------------------------//
unsigned short NetworkObject::staticItemID = 1;
...
NetworkObject::NetworkObject( bool bAutoAssignID )
{

    m_bAlreadyInitialized = false;

    // boto: for client objects no auto-assigning should be done!
    m_pkEntity = NULL;
    if ( bAutoAssignID == false ) {

        serverAssignedID = false;
        objectID = UNASSIGNED_OBJECT_ID;
        return;
