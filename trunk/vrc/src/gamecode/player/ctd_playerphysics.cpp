...
{
                            PlayerRayCastData( NewtonBody* me )
                            {
                                _p_body    = me;
                                _parameter = 1.2f;
                            }
...
int playerContactProcess( const NewtonMaterial* p_material, const NewtonContact* p_contact )
{
    ...
    PlayerPhysics* p_phys   = dynamic_cast< PlayerPhysics* >( p_entity );
    if ( !p_phys )
    {
        s_colStruct->_p_otherEntity = p_entity;
        p_body = s_colStruct->_p_body1;
        p_phys = static_cast< PlayerPhysics* >( NewtonBodyGetUserData( p_body ) );
    }
    else
    {
...
float PlayerPhysics::findFloor( NewtonWorld* world, const Vec3f& p0, float maxDist )
{
...
void PlayerPhysics::physicsBodyDestructor( const NewtonBody* p_body )
{
}

// transformation callback
void PlayerPhysics::physicsSetTransform( const NewtonBody* p_body, const float* p_matrix )
{
    EnPlayer* p_node = static_cast< PlayerPhysics* >( NewtonBodyGetUserData( p_body ) )->_p_player;
...
int PlayerPhysics::collideWithLevel( const NewtonMaterial* p_material, const NewtonContact* p_contact )
{
	Vec3f point;
	Vec3f normal;
	Vec3f velocity;
...
int PlayerPhysics::collideWithOtherEntities( const NewtonMaterial* p_material, const NewtonContact* p_contact )
{ 
    //! TODO
    return 1;
}

void PlayerPhysics::physicsApplyForceAndTorque( const NewtonBody* p_body )
{
    float mass;
    float Ixx;
    float Iyy;
    ...
    PlayerPhysics* p_phys = static_cast< PlayerPhysics* >( NewtonBodyGetUserData( p_body ) );
    Vec3f force  = p_phys->_p_player->_force;
...
float PlayerPhysics::physicsRayCastPlacement( const NewtonBody* p_body, const float* p_normal, int collisionID, void* p_userData, float intersectParam )
{
    float paramPtr = 1.2f;
...
//---------------------------------
PlayerPhysics::PlayerPhysics( EnPlayer* p_player ) :
_p_player( p_player ),
_p_world( Physics::get()->getWorld() ),
_stepHeight( p_player->_stepHeight ),
_isAirBorne( false ),
_playerHeight( p_player->_dimensions._v[ 2 ] ),
_jumpTimer( 0 ),
_jumpForce( 50.0f ),
_climbForce( 30.0f ),
_gravity( Physics::get()->getWorldGravity() )
{ 
    // give this object the same name as 
    setInstanceName( p_player->getInstanceName() );
...
void PlayerPhysics::initialize()
{
    ...
    _stepHeight   = _p_player->_stepHeight;
    _playerHeight = _p_player->_dimensions._v[ 2 ] * 2.0f;
    _gravity      = _p_player->_gravity;

    // create the collision 
    NewtonCollision *p_collision = NewtonCreateSphere( _p_world, _p_player->_dimensions._v[ 0 ], _p_player->_dimensions._v[ 1 ], _p_player->_dimensions._v[ 2 ], NULL); 
    ...
    NewtonBodySetLinearDamping( _p_body, _p_player->_linearDamping );
    ...
    NewtonBodySetMassMatrix( _p_body, mass, 1.0f, 1.0f, 1.0f );
    ...
    mat.ptr()[ 14 ] = z + _p_player->_dimensions._v[ 2 ] + 0.2f; // add an offset of player height plus 0.2 meters
